...
struct PortugueseLessonPlan: LessonPlan {
    
    //MARK: - Read-only properties
    let languageName = "Portuguese"
    let topics = Data.portugueseTopics
    
    //MARK: - Mutable Properties
    
    var results: [Results] = PortugueseLessonPlan.readResultsRecords()
    
    //MARK: - Helpers
    
    mutating func toggleLessonRead (for title: String) {
        if let index = results.firstIndex(where: {$0.topicTitle == title }) {
            results[index].isLessonRead.toggle()
            UserDefaults.standard.set(results[index].isLessonRead, forKey: key(for: title, type: Key.isLessonRead))
        } else {
    ...
    private static func readResultsRecords () -> [Results] {
        ...
        Data.portugueseTopics.forEach { topic in
            var resultRecord = Results(topicTitle: topic.title)
            
            resultRecord.isLessonRead = UserDefaults.standard.bool(
    ...
    private struct Key {
        static let isLessonRead = "lesson"
        static let isFlashcardsCompleted = "flashcard"
        static let isQuizzesCompleted = "quiz"
        static let quizScore = "score"
    }
    
    private struct Data {
        static let portugueseTopics = [
            Topic(
